...
void typp(const char* podflaga){
	...
	int limitslowa=0;
	int i=0;
	for (int j=2; j<strlen(podflaga);j++){
	    c[i]= podflaga[j];
	    i++;
	}
	limitslowa = atoi(c);
	char slowo[100];
	while(fgets(slowo,sizeof(slowo),plik1)){		
			for(int i=0;i<(strlen(slowo)-1)/2;i++){
				palindrom = true;
				if(slowo[i]!=slowo[(strlen(slowo))-1-(1+i)]){
					palindrom = false;
					break;
				}
			}
        ...
        if(palindrom == true && strlen(slowo)>=limitslowa+1){
			fputs(slowo, plik2);
		}
    }	
	fclose(plik1);
